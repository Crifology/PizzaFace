...
scene('lose', ({score}) => {
    add([text('YOU FELL! SCORE: ' + score, 32), origin('center'), pos(width()/2, height()/2)]);
})
scene('lose2', ({score}) => {
    add([text('UNDEAD ATE YOU! SCORE: ' + score, 32), origin('center'), pos(width()/2, height()/2)]);
})
